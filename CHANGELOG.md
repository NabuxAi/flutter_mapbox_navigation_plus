## Unreleased
* **Android: enabling offline no longer blanks the map outside downloaded regions** — `MapboxOfflineManager.initialize()` set the renderer to `READ_AND_UPDATE`, which requests whole tile *packs* from the network and otherwise reads only the tile store. Anywhere the user had not downloaded a region the map drew nothing, and because the same store backs the routing tiles, routes through those areas could not be built either ("could not find a valid route"). An app that calls `enableOfflineRouting` once per session therefore lost the whole world outside its downloaded city. The mode is now `READ_ONLY`: downloaded packs are used where they exist, and everywhere else falls back to ordinary per-tile network requests.
* **Lane guidance** — `RouteProgressEvent.lanes` now carries the lanes of the road ahead, left to right, with the turns painted in each and which of them can be used for the manoeuvre being approached (`LaneGuidance`). This is one of the things the v2 drop-in `NavigationView` drew for free and the v3 bare-map path did not, so a custom Flutter HUD had no way to show it. Android reads the banner's sub-instruction, iOS the tertiary instruction, and both send the same field names and the same direction vocabulary so one Dart model reads either. Empty away from a junction, so a UI can hide the strip rather than reserve space for it.
